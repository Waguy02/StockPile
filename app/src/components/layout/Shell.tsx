...
export function Layout({ children, currentView, onNavigate }: LayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-slate-50/50 text-slate-900 font-sans overflow-hidden">
      {/* Sidebar - Desktop */}
      <aside className="hidden md:flex flex-col w-72 bg-[#0B1121] text-white border-r border-slate-800/50 shadow-2xl z-20">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        {/* Header */}
        <header className="h-20 bg-white/80 backdrop-blur-xl border-b border-slate-200/60 flex items-center justify-between px-6 md:px-10 z-10 sticky top-0">
          <div className="flex items-center md:hidden">
            ...
            <span className="ml-3 text-lg font-bold text-slate-800">StockPILE</span>
          </div>
          ...
          <div className="flex-1 px-8 py-5 flex justify-end items-center gap-6">
            ...
            <button className="relative p-2.5 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-full transition-all duration-200">
              ...
              <span className="absolute top-2 right-2.5 w-2 h-2 bg-rose-500 rounded-full border-2 border-white ring-1 ring-rose-500/20"></span>
            </button>
          </div>
        </div>
      </header>
